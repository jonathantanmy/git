...
#define LARGEST_ENTRY (GIT_SHA1_RAWSZ + 10)
static void write_oversized_info(int fd)
{
	struct oversized_blob **obs;
	struct hashmap_iter iter;
	struct oversized_blob *ob;
	int i = 0;
	unsigned char buffer[LARGE_PACKET_DATA_MAX];
	unsigned char *next = buffer;

	ALLOC_ARRAY(obs, oversized_blobs.size);
	...
	next += encode_varint(oversized_blobs.size, next);
	for (i = 0; i < oversized_blobs.size; i++) {
		if (next - buffer > LARGE_PACKET_DATA_MAX - LARGEST_ENTRY) {
			if (packet_write_gently(fd, (char *) buffer,
						next - buffer))
				die("Could not write oversized info");
			next = buffer;
		}
		memcpy(next, &obs[i]->oid, sizeof(obs[i]->oid));
		next += sizeof(obs[i]->oid);
		next += encode_varint(obs[i]->size, next);
	}
	if (packet_write_gently(fd, (char *) buffer, next - buffer))
		die("Could not write oversized info");
}
