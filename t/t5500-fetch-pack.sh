...
test_expect_success '--blob-max-bytes sets promised blobs' '
	rm -rf server client &&
...
test_expect_success 'tolerate empty promised blobs list' '
	rm -rf server client &&
	test_create_repo server &&
	test_commit -C server one &&
	test_config -C server uploadpack.advertiseblobmaxbytes 1 &&
	test_create_repo client &&
	git -C client fetch-pack --blob-max-bytes=10000000 ../server HEAD &&

	# Nothing is stored
	test $(cat client/.git/objects/promisedblob | wc -c) -eq 0
'
