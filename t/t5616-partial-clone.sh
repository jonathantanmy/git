...
# we may get multiple packfiles (one for each blob/commit).
test_expect_success 'verify blame causes dynamic object fetch' '
	git -C pc1 blame origin/master -- file.1.txt >observed.blame &&
	test_cmp expect.blame observed.blame
'
# TODO WIP the above blame fails to properly credit the "Mod [abc]" lines,
# TODO WIP but the second time we run it, it does.  The first invocation
# TODO WIP does not fetch all off the intermediate blobs, but the second
# TODO WIP one does.
test_expect_success 'verify blame causes dynamic object fetch' '
	git -C pc1 blame origin/master -- file.1.txt >observed.blame &&
	test_cmp expect.blame observed.blame
